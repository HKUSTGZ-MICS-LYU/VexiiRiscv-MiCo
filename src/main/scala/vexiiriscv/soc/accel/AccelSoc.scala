...
class AccelSoc(p : AccelSocParam) extends Component {
  ...
  val system = new ClockingArea(socCtrl.system.cd) {
    ...
    val accel = new AccelCoreFiber(new AccelCoreParam())
    mainBus << accel.down
    accel.up at 0x40000000l of mainBus
    ...
    val peripheral = new Area {
      ...
      plic.mapUpInterrupt(3, accel.interrupt)
